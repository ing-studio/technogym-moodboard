...
- `scripts/build_standalone.py [deck/index.html] [-o index.html]`: bundle the deck into one
  portable HTML file, every picture inlined as a data: URI and unused catalogue entries dropped. That file
  is what travels to a client and what GitHub Pages serves from the repository root; rebuild it, never
  edit it. `verify.py` checks a bundle too.
